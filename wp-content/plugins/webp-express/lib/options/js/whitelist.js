...
<html><body><p>function updateWhitelistInputValue() {
...
function whitelistSetHTML() {
    ...
    if (window.whitelist &amp;&amp; window.whitelist.length &gt; 0) {
        ...
        s+='</p><ul>';
        for (var i=0; i<window.whitelist.length i s>';
            s+= webpexpress_escapeHTML(window.whitelist[i]['label']);
            s+='<div class="whitelist-links">'
            s+='<a href="javascript:whitelistEditEntry('%20+%20i%20+%20')">edit</a>';
            s+='<a href="javascript:whitelistRemoveEntry('%20+%20i%20+%20')">remove</a>';
            s+='</div>'
            s+='';
        }
        s+='</window.whitelist.length></ul>';
    } else {
