...
<html><body><p>let scrollData = true;
let scrollOffset = 30;
...
UAGBTableOfContents = {
	...
	parseTocSlug( slug ) {
		...
		const parsedSlug = slug
			.toString()
			.toLowerCase()
			.replace( /\&hellip;+/g, '' ) // Remove multiple &hellip;
			...
			.replace( /[&\/\\#,^!+()$~%.\[\]'":*?;-_<>{}@&lsquo;&rsquo;&rdquo;&ldquo;|]/g, '' ) // Remove special chars
